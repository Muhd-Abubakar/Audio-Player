...
async function playSongs() {
  try {
    const searchTerm = searchInput.value.trim().split(' ').join('+');
...
speechButton.addEventListener('click', () => {
  startSpeechRecognition((transcript) => {
    searchInput.value = transcript; // Update the input field with the recognized speech
    playSongs(); // Trigger the song search and play
  });
});
*/
// Import the speech recognition function
import { startSpeechRecognition } from './speecRecog.js';
